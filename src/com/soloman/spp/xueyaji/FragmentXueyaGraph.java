...
public class FragmentXueyaGraph extends Fragment {
			...
			if (cursor!=null && cursor.getCount() > 0){
				...
				if ("sys".equals(mode)){
					...
					while (!cursor.isAfterLast()){					
						...
						value[(int) (x -1)] = y;//高压 sys
						unit[(int) (x -1)] = cursor.getString(1);
						date[(int) (x -1)] = cursor.getString(5);
						
						cursor.moveToNext();
					}
				}else if ("dia".equals(mode)){
					...
					while (!cursor.isAfterLast()){					
						...
						value2[(int) (x -1)] = cursor.getString(3);//低压dia
						unit2[(int) (x -1)] = cursor.getString(1);
						date2[(int) (x -1)] = cursor.getString(5);
						
						cursor.moveToNext();
					}
				}else if ("pul".equals(mode)){
					value3 = new String [cursor.getCount()];//心率pul
					unit3 = new String [cursor.getCount()];
					date3 = new String [cursor.getCount()];
					while (!cursor.isAfterLast()){					
						x++;
						y = cursor.getString(4);
						mCurrentSeries.add(x,  Integer.valueOf(y));
						
						value3[(int) (x -1)] = cursor.getString(3);//低压dia
						unit3[(int) (x -1)] = "/min";
						date3[(int) (x -1)] = cursor.getString(5);
...
//	    }
        ...
        if ("sys".equals(mode)){
	    	...
	    	renderer.setColor(Color.BLACK); //折线颜色
	    }
	    ...
	    seriesTitle2 = "DIA";// + (mDataset.getSeriesCount() + 1);//图例  
	    ...
	    seriesTitle3 = "PUL";// + (mDataset.getSeriesCount() + 1);//图例  
	     
	    XYSeries series3 = new XYSeries(seriesTitle3);//定义XYSeries
	    mDataset.addSeries(series3);//在XYMultipleSeriesDataset中添加XYSeries  
	    mCurrentSeries = series3;//设置当前需要操作的XYSeries  \	    
        
        addPoint("pul");
        
        XYSeriesRenderer renderer3 = new XYSeriesRenderer();//定义XYSeriesRenderer  
        mRenderer.addSeriesRenderer(renderer3);//将单个XYSeriesRenderer增加到XYMultipleSeriesRenderer
        
        renderer3.setPointStyle(PointStyle.CIRCLE);//点的类型是圆形  
        renderer3.setFillPoints(true);//设置点是否实心  
	    renderer3.setColor(Color.BLACK); //折线颜色
...
//	            		  " Y = " + String.valueOf(seriesSelection.getValue()));
	            	...
	            	if (seriesSelection.getSeriesIndex() == 0){//sys series
	            		tv_graphdata.setText(appState.userID + "  " 
	  	            		  + appState.userName + "\n"
	  	            		  + "SYS:" + value[(int) seriesSelection.getXValue() - 1] + unit[(int) seriesSelection.getXValue() - 1] + ","
	  	            		  + "DIA:" + value2[(int) seriesSelection.getXValue() - 1] + unit[(int) seriesSelection.getXValue() - 1] + ","
	  	            		  + "PUL:" + value3[(int) seriesSelection.getXValue() - 1] + "/min\n"
