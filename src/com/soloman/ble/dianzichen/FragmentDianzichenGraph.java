...
public class FragmentDianzichenGraph extends Fragment {
	...
	public String [] value,value2;	//   value
	public String [] unit, unit2;	//   unit 
	public String [] date, date2;	//  date
	...
	public void addPoint(String mode ){
		...
		if (!"".equals(appState.userID)){
			cursor = appState.database.getDianzichen(appState.userID);	//电子秤
			if (cursor!=null && cursor.getCount() > 0){
				...
				if ("weight".equals(mode)){
					...
					while (!cursor.isAfterLast()){					
						x++;
						y = cursor.getFloat(cursor.getColumnIndex("weight"));
						...
						unit[(int) (x - 1)] = cursor.getString(cursor.getColumnIndex("unit"));
						date[(int) (x - 1)] = cursor.getString(cursor.getColumnIndex("date"));
						
						cursor.moveToNext();
					}
				}else if ("bmi".equals(mode)){
					value2 = new String [cursor.getCount()];
					unit2 = new String [cursor.getCount()];
					date2 = new String [cursor.getCount()];
					while (!cursor.isAfterLast()){					
						x++;
						y = cursor.getFloat(cursor.getColumnIndex("bmi"));
						BigDecimal bd = new BigDecimal(y);
						bd = bd.setScale(1,BigDecimal.ROUND_HALF_UP);  
//						y = bd.floatValue();
						mCurrentSeries.add(x,  bd.floatValue());
						
						value2[(int) (x -1)] = String.valueOf( bd.floatValue());
//						unit2[(int) (x -1)] = cursor.getString(2);
						date2[(int) (x -1)] = cursor.getString(cursor.getColumnIndex("date"));
...
//	    }
	    ...
	    seriesTitle2 = "BMI";// + (mDataset.getSeriesCount() + 1);//图例  
...
//	            		  " Y = " + String.valueOf(seriesSelection.getValue()));
	            	...
	            	if (seriesSelection.getSeriesIndex() == 0){//weight series
	            		tv_graphdata.setText(appState.userID + "  " 
	  	            		  + appState.userName + "\n"
	  	            		  + "Weight:" + value[(int) seriesSelection.getXValue() - 1] + unit[(int) seriesSelection.getXValue() - 1] + ","
	  	            		  + "BMI:" + value2[(int) seriesSelection.getXValue() - 1] + "\n"
	  	            		  + date[(int) seriesSelection.getXValue() - 1]);
	            	}else if (seriesSelection.getSeriesIndex() == 1){//bmi series
	            		tv_graphdata.setText(appState.userID + "  " 
		  	            		  + appState.userName + "\n"
		  	            		  + "Weight:" + value[(int) seriesSelection.getXValue() - 1] + unit[(int) seriesSelection.getXValue() - 1] + ","
		  	            		  + "BMI:" + value2[(int) seriesSelection.getXValue() - 1] + "\n"
