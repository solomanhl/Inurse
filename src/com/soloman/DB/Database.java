...
//	4:加入电子秤、病人表增加性别、身高、体重、出生年月
	...
	private String table_dianzichen = "dianzichen";// 电子秤测量记录
	private String table_xueyang = "xueyang";	//血氧仪
	...
	public class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("create table IF NOT EXISTS " + table_patient + 
					...
					"tizhong int, " +	//体重
					"age varchar(10), " +	//年龄
					"chushennian varchar(10), " +	//出生年
					"chushenyue varchar(10), " +	//出生月
					"chushenri varchar(10) )");	//出生日	
			...
			db.execSQL("create table IF NOT EXISTS " + table_value + 
					"(id varchar(10) ," + //病人id
					"devicetype varchar(10), " + // 设备类型，1蓝牙温度计  2usb女性体温计
			...
			//电子秤
			db.execSQL("create table IF NOT EXISTS " + table_dianzichen + 
					"(id varchar(10) ," + //病人id
					"devicetype varchar(2), " + // 设备类型，1宝通电子秤
					"weight varchar(10), " + // 重量
					"unit varchar(10), " + // 重量单位kg   lb
					"fat varchar(10), " + // 
					"water varchar(10), " + // 
					"muscle varchar(10), " + // 
					"bon varchar(10), " + //
					...
					"note varchar(100) )"); //每条记录的note，医嘱
			SQLdb = db;
			System.out.println("DB onCreate --- table_dianzichenueya");			
			
			//血氧仪
			db.execSQL("create table IF NOT EXISTS " + table_xueyang + 
					"(id varchar(10) ," + //病人id
					"devicetype varchar(10), " + // 设备类型，1血氧仪
					"o2 varchar(10), " + // 血氧浓度（%）
					"pul varchar(10), " + // 心率  pulse/min
					"tep varchar(10), " + // 温度
					"date varchar(50), " + // 测量日期
					...
					"note varchar(100) )"); //每条记录的note，医嘱
			SQLdb = db;
			System.out.println("DB onCreate --- table_xueyang");			
		}
	...
	public Cursor get_patient(){
		...
		try {
			cursor = SQLdb.query(table_patient, // table名
					new String[] { "id", "firstname", "lastname", "tel", "mail","note",
						"sex", "shengao", "tizhong", "chushennian", "chushenyue", "chushenri", "age"}, // 字段
					null, // 条件
					null, 
					null, //group by
					null, //having
	...
	public Cursor get_patient_ext(String id){
		Cursor cursor = null;
		try {
			cursor = SQLdb.query(table_patient, // table名
					new String[] { "id", "sex", "shengao", "tizhong", "chushennian", "chushenyue","chushenri"}, // 字段
					"id = '" + id + "'", // 条件
					null, 
					null, //group by
					null, //having
	...
	// 修改信息
	public int Update_patient_ext(String id, String sex, int shengao, String tizhong,
			String chushennian, String chushenyue, String chushenri) {
		ContentValues cv = new ContentValues();
		cv.put("sex", sex);
		cv.put("shengao", shengao);
		cv.put("tizhong", tizhong);
		cv.put("chushennian", chushennian);
		cv.put("chushenyue", chushenyue);
		cv.put("chushenri", chushenri);
		String[] args = { String.valueOf(id) };
		return SQLdb.update(table_patient, cv, "id=?", args);
	}
	
	// 修改信息
	public int Update_patient_ext(String id, String sex, String age, String shengao) {
		ContentValues cv = new ContentValues();
		cv.put("sex", sex);
		cv.put("shengao", shengao);
		cv.put("age", age);
	...
	//-------------------------------------------------------------------------------------
	//电子秤
	public long addDianzichen(String id, String devicetype, String weight,
			String unit, String fat, String water, String muscle, String bon,
			String kcal, String bmi, String date, String time ) {
		ContentValues cv = new ContentValues();
		...
		System.out.println("DB.add_dianzichendata " + table_dianzichen);
		return SQLdb.insert(table_dianzichen, null, cv);
	}

	public Cursor getDianzichen(String id) {
		Cursor cursor = null;
		try {
			cursor = SQLdb.query(table_dianzichen, // table名
					new String[] { "id", "weight", "unit", "fat", "water", "muscle",
									"bon", "kcal", "bmi",
									"date", "time", "note" }, // 字段
					"id = '" + id + "'", // 条件
					null, null, null, null);
	...
	// 修改信息
	public int updateDianzichen(String id, String date, String note) {
		ContentValues cv = new ContentValues();
		cv.put("note", note);
		String[] args = { id, date };
		return SQLdb.update(table_dianzichen, cv, "id=? and date=?", args);
	}

	public long delDianzichen(String date) {
		return SQLdb.delete(table_dianzichen, "date = '" + date + "'", null);
	}
	...
	//-------------------------------------------------------------------------------------
	//血氧
		public long addXueyang(String id, String devicetype,  String o2,  String pul, String tep, String date, String time ) {
			ContentValues cv = new ContentValues();
			...
			System.out.println("DB.add_xueyang " + table_xueyang);
			return SQLdb.insert(table_xueyang, null, cv);
		}
		
		public Cursor getXueyang(String id ) {
			Cursor cursor = null;
			try {
				cursor = SQLdb.query(table_xueyang, // table名
						new String[] { "id",  "o2",  "pul", "tep", "date","time", "note" }, // 字段
						"id = '" + id + "'", // 条件
						null, null, null, null);
			} catch (Exception e) {
				e.printStackTrace();
		...
		// 修改信息
		public int updateXueyang(String id, String date, String note) {
			ContentValues cv = new ContentValues();
			cv.put("note", note);
			String[] args = { id, date };
			return SQLdb.update(table_xueyang, cv, "id=? and date=?", args);
		}
		
		public long delXueyang(String date) {
			return SQLdb.delete(table_xueyang, "date = '" + date + "'",
					null);
		}
